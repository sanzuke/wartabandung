<script>
function publish(id,sts){
	$.ajax({
	type : 'POST',
	url  : '<?php echo BASE_PATH_ADMIN ?>publish.php',
	data : 'ID='+id+'&STS='+sts,
...
echo"<h2 style='margin:0'>Komentar</h2>";
echo $msg;
require("page-number.php");
	...
	$pagenumber->query	= "select count(*) from komentar";
	//Show PageNumber
	...
	if($pagenumber->TotalRecord){
		$result = mysql_query("select * from komentar order by id desc limit {$pagenumber->start}, {$pagenumber->limit}");
		if (!$result){
			echo mysql_error();
		}
		$i=1;
		...
		while($r = mysql_fetch_array($result)){
			...
			if($r['publish']=='1'){ $cek ='checked="checked"'; } else { $cek ='';}
			echo"
			<td>$r[nama]</td>
			<td>$r[email]</td>
			<td>$r[pesan]</td>
			<td><input type=\"checkbox\" id=\"publish\" name=\"publish\" $cek onclick=\"publish('$r[id]','$r[publish]')\" ></td>
			<td width=\"12\"><a href=\"dashboard.php?page=komentar&op=del&id=$r[0]\"><img src=\"images/8.png\"></a></td></tr>";
			$i++;
			} 
	}
		echo ("</table>");
